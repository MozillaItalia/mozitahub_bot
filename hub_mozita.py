...
def risposte(msg):
    ...
    home = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='Vai al gruppo Home',
                              url='https://t.me/joinchat/BCql3UMy26nl4qxuRecDsQ')],
    ])

    feedback = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Vai in 'Home' e lascia il tuo feedback",
                              url='https://t.me/joinchat/BCql3UMy26nl4qxuRecDsQ')],
        [InlineKeyboardButton(text="Chiedi di voler contribuire al bot in 'Home'",
                              url='https://t.me/joinchat/BCql3UMy26nl4qxuRecDsQ')]
    ])

    start = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text='Mostrami che cosa posso fare', callback_data='/help')],
        [InlineKeyboardButton(
    ...
    supporto = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='Leggi le FAQ dal forum di Mozilla Italia',
    ...
    gruppi = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(
            text='Voglio diventare volontario', url='https://t.me/joinchat/B1cgtEQAHkGVBTbI0XPd-A')],
    ...
    news = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text="Vai al canale ufficiale 'News'", url='https://t.me/mozItaNews')],
    ])

    forum = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text='Vai al forum di Mozilla Italia', url='https://forum.mozillaitalia.org/')],
    ])

    call = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='Vedi tutte le call',
                              callback_data='/listacall')],
        [InlineKeyboardButton(
            text='Scopri quando sarà la prossima call', callback_data='/prossimacall')],
    ])

    load_listaCall = []
    for x in call_mensili_list:
        load_listaCall.append([InlineKeyboardButton(
            text=str(x), url=str(call_mensili_list[x]))])
    ...
    regolamento = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text='Leggi Regolamento', url='https://github.com/Sav22999/Guide/blob/master/Mozilla%20Italia/Telegram/regolamento.md')],
    ])

    avvisi = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='Attiva avvisi',
                              callback_data="/avvisiOn")],
        [InlineKeyboardButton(
            text='Disattiva avvisi', callback_data="/avvisiOff")],
    ])
    ...
    # elif text=="/stop":
        ...
        bot.sendMessage(chat_id, "Scegli tu cosa vuoi fare :)",
                        reply_markup=supporto)
    elif text == "/gruppi":
        ...
        bot.sendMessage(
            chat_id, "Sai già come potresti essere utile e contribuire a Mozilla Italia?", reply_markup=collabora)
    elif text == "/vademecum":
        ...
        bot.sendMessage(
            chat_id, "Allora, che cosa vorresti fare?", reply_markup=help)
    elif text == "/news":
        ...
        bot.sendMessage(
            chat_id, "Entra a far parte del gruppo dedicato agli sviluppatori Mozilla Italia.", reply_markup=developer)
    elif text == "/design":
        bot.sendMessage(
        ...
        bot.sendMessage(
            chat_id, "Questo è tutte l'elenco delle call già tenute, con il relativo link per poterle guardare.", reply_markup=listaCall)
    elif text == "/prossimacall":
        bot.sendMessage(chat_id, "La prossima call comunitaria sarà quella del "+giornoCall+" "+meseCall+" "+annoCall +
                        ", (il primo venerdì del mese) alle ore 18:30.\nQuesta è una stima, potrebbero esserci slittamenti o annullamenti. Per maggiore sicurezza chiedi nel gruppo Home di Mozilla Italia.")
    elif text == "/progetti":
        bot.sendMessage(
            chat_id, "Questi sono i progetti di mozilla attualmente attivi:", reply_markup=progetti)
        bot.sendMessage(
            chat_id, "Questi, invece, sono i progetti della comunità di mozilla italia:", reply_markup=progettimozita)
    elif text == "/regolamento":
        bot.sendMessage(
            chat_id, "Leggi il regolamento vigente nei gruppi comunitari di Mozilla Italia:", reply_markup=regolamento)
    elif text == "/avvisi":
        ...
        if not (user_id in avvisi_on_list):
            ...
            try:
                ...
                bot.sendMessage(
                    chat_id, "Si è verificato un errore imprevisto durante l'attivazione degli avvisi.")
        else:
        ...
        if user_id in avvisi_on_list:
            ...
            try:
                ...
                bot.sendMessage(
                    chat_id, "Si è verificato un errore imprevisto durante la disattivazione degli avvisi.")
        else:
        ...
        if status_user == "A":
            ...
            bot.sendMessage(
                chat_id, "Non sei un admin, pertanto non hai i permessi per utilizzare questa azione.")
    else:
        bot.sendMessage(
            chat_id, "Questo comando non è stato riconosciuto.", reply_markup=start)
    ...
    if admin:
        ...
        if(azione[0] == "/admin" and len(azione) >= 2):
            if(azione[1] == "help" and len(azione) == 2):
                ...
                bot.sendMessage(
                    chat_id, "Messaggio inviato correttamente a tutti gli utenti iscritti alle news.")
                ...
                if(azione[2] == "aggiungi"):
                    ...
                    if not (nome in call_mensili_list):
                        ...
                        bot.sendMessage(
                            chat_id, "La call mensile '"+str(nome)+"' è già presente.")
                    ...
                    if nome in call_mensili_list:
                        ...
                        try:
                            with open(call_mensili_list_path, "wb") as f:
                                f.write(json.dumps(
                                    call_mensili_list).encode("utf-8"))
                            bot.sendMessage(
                                chat_id, "Call mensile '"+str(nome)+"' ("+str(link)+") modificata correttamente.")
                        except Exception as e:
                        ...
                        bot.sendMessage(
                            chat_id, "La call mensile '"+str(nome)+"' non è stata trovata.")
                        ...
                        try:
                            with open(call_mensili_list_path, "wb") as f:
                                f.write(json.dumps(
                                    call_mensili_list).encode("utf-8"))
                            bot.sendMessage(
                                chat_id, "Call mensile '"+str(nome)+"' eliminata correttamente.")
                        except Exception as e:
                ...
                if(azione[3] == "aggiungi"):
                    ...
                    if not (temp_chat_id in avvisi_on_list):
                        ...
                        try:
                            ...
                            bot.sendMessage(
                                chat_id, "La chat_id '"+str(temp_chat_id)+"' è stata inserita correttamente.")
                        except Exception as e:
                            print("Excep:12 -> "+str(e))
                            bot.sendMessage(
                                chat_id, "Si è verificato un errore inaspettato e non è possibile salvare 'avvisi_on_list.json'.")
                    else:
                    ...
                    if temp_chat_id in avvisi_on_list:
                        ...
                        try:
                            with open(avvisi_on_list_path, "wb") as f:
                                f.write(json.dumps(
                                    avvisi_on_list).encode("utf-8"))
                            bot.sendMessage(
                                chat_id, "La chat_id '"+str(temp_chat_id)+"' è stata eliminata correttamente.")
                        except Exception as e:
                        ...
                        bot.sendMessage(chat_id, "La chat_id '" +
                                        str(temp_chat_id)+"' non è stata trovata.")
                else:
                    admin_err1 = True
                ...
                if(azione[3] == "aggiungi"):
                    ...
                    if not (nome in progetti_mozita_list):
                        ...
                        try:
                            ...
                            bot.sendMessage(
                                chat_id, "Progetto comunitario '"+str(nome)+"' ("+str(link)+") inserito correttamente.")
                        except Exception as e:
                            print("Excep:14 -> "+str(e))
                            bot.sendMessage(
                                chat_id, "Si è verificato un errore inaspettato e non è possibile salvare 'progetti_mozita_list.json'.")
                    else:
                        bot.sendMessage(
                            chat_id, "Il progetto comunitario '"+str(nome)+"' è già presente.")
                    ...
                    if nome in progetti_mozita_list:
                        ...
                        try:
                            with open(progetti_mozita_list_path, "wb") as f:
                                f.write(json.dumps(
                                    progetti_mozita_list).encode("utf-8"))
                            bot.sendMessage(
                                chat_id, "Progetto comunitario '"+str(nome)+"' ("+str(link)+") modificato correttamente.")
                        except Exception as e:
                        ...
                        bot.sendMessage(
                            chat_id, "Il progetto comunitario '"+str(nome)+"' non è stato trovato.")
                elif(azione[3] == "elimina"):
                    del azione[0]
                    del azione[0]
                    del azione[0]
                ...
                if(azione[2] == "aggiungi"):
                    ...
                    if not (nome in progetti_list):
                        ...
                        try:
                            ...
                            bot.sendMessage(
                                chat_id, "Si è verificato un errore inaspettato e non è possibile salvare 'progetti_list.json'.")
                    else:
                ...
                if(azione[2] == "aggiungi"):
                    ...
                    if not (nome in collaboratori_hub):
                        ...
                        try:
                            ...
                            bot.sendMessage(
                                chat_id, "Si è verificato un errore inaspettato e non è possibile salvare 'collaboratori_hub.json'.")
                    else:
                        bot.sendMessage(
                            chat_id, "'"+str(nome)+"' è già presente nella lista dei collaboratori.")
                elif(azione[2] == "elimina"):
                    del azione[0]
                    del azione[0]
                    del azione[0]
                    ...
                    if nome in collaboratori_hub:
                        ...
                        bot.sendMessage(
                            chat_id, "'"+str(nome)+"' non è presente nella lista dei collaboratori.")
                else:
                    admin_err1 = True
            else:
                admin_err1 = True
